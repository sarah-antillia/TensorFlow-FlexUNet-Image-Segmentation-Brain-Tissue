...
class ImageMaskDatasetGenerator:
  ...
  def colorize_mask(self, j,mask):
    m1 = mask[j,:,:, 1]
    m2 = mask[j,:,:, 2]
    m3 = mask[j,:,:, 3]
    h, w = m1.shape[:2]
    colorized = np.zeros((h, w, 3), dtype=np.uint8) 
    #rgb_colors = (180,40,40):1, (40,128,40):2, (255,255,0):3

    #Colorize maskk
    colorized[np.equal(m1,1)] = (40,40,180)  #bgr brown
    colorized[np.equal(m2,1)] = (40,128,40)  #bgr dark green
    colorized[np.equal(m3,1)] = (0,255,255)  #bgr yellow
  ...
  def generate(self, images_dir, masks_dir, output_images_dir, output_masks_dir): 
    ...
    for i in range(num):
        ...
        for j in range(n):
          img = image[j,:,:,0] 
          print(img.shape)
          min1, max1 = img.min(), img.max()
          if max1 > min1:
            img = (img - min1) / (max1 - min1) * 255.0
          else:
            img = img * 0
          ...
          cv2.imwrite(out_imagefilepath, img)
          print("Savd {}".format(out_imagefilepath))

          # The mask has 4 segments ( one is background).
          colorized = self.colorize_mask(j, mask)
          ...
          if self.augmentation:
            ...
            self.augment(img,       out_filename, output_images_dir, border=(0, 0, 0),  mask=False)
...
if __name__ == "__main__":
  try:
    images_dir    = "./ibsr_3d/train/images"   
    masks_dir    = "./ibsr_3d/train/mask"


    output_dir   = "./Augmented-Brain-Tissue"
    ...
    images_dir   = "./ibsr_3d/valid/images"   
    masks_dir    = "./ibsr_3d/valid/mask"
    generator.generate(images_dir,
                        masks_dir,
                        output_images_dir, 
                        output_masks_dir,)
